...
function drawSkybox() {
  if (!gl || !program || !gl.isTexture(skyTexture)) {
    console.warn("⚠️ Skybox texture not loaded or WebGL not ready");
    return false;
  }

  try {
    const scale = [2000, 2000, 2000];
    const position = [0, 0, 100];
    return drawCube(scale, position, skyTexture);
  } catch (error) {
    console.error("❌ Error drawing skybox:", error);
    return false;
  }
}
...
function drawFloor() {
  const scale = [30, 30, 1.7];
  const position = [0, 0, 0];  // z=0 για να "κάθεται" στο επίπεδο
  drawCube(scale, position, floorTexture);
}
